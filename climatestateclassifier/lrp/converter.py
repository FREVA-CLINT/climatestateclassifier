...
from climatestateclassifier.model.decoder import Decoder
...
def convert_net(module, modules=None):
    # First time
    if modules is None:
        modules = []
        for m in module.children():
            convert_net(m, modules=modules)
    ...
    # Recursion
    if isinstance(module, Decoder) or isinstance(module, Encoder):
        convert_net(module.main, modules=modules)
    elif isinstance(module, EncoderBlock):
        convert_net(module.conv, modules=modules)
        if hasattr(module, "pooling"):
            convert_net(module.pooling, modules=modules)
        if hasattr(module, "bn"):
            convert_net(module.bn, modules=modules)
        if hasattr(module, "activation"):
            convert_net(module.activation, modules=modules)
